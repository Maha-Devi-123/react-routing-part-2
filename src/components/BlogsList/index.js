...
class BlogsList extends Component {
  state = {currentBlogsData: [], isLoading: true}
  ...
  getBlogsData = async () => {
    ...
    this.setState({currentBlogsData: modifiedData, isLoading: false})
  }

  render() {
    const {currentBlogsData, isLoading} = this.state
    return (
      <div className="blog-list-container">
        {isLoading ? (
          <Loader type="TailSpin" color="#00BFFF" height={50} width={50} />
        ) : (
          currentBlogsData.map(item => (
